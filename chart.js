...
function getVirtulData(year) {
  year = year || '2017';
  let date = +echarts.number.parseDate(year + '-03-01');
  let end = +echarts.number.parseDate(year + '-03-30');
  ...
  for (let time = date; time < end; time += dayTime) {
    ...
    for (let i = 0; i < eventCount; i++) {
      items.push(Math.round(Math.random() * (200 - 1)));
      if (echarts.format.formatTime('yyyy-MM-dd', time) == '2017-03-05')
      {
        items.push(['W22']);
        items.push([1]);
      }
      ...
      if (echarts.format.formatTime('yyyy-MM-dd', time) == '2017-03-06')
      {
        items[2] = '1';
      }
    }
